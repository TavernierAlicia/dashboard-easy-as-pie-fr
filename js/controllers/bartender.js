export default function home() {
    ...
    const init = (context, params) => {
        ...
        if (!token) return app.router.resolve('/login');
        console.log("http");
        app.services.http().post("/bartender", {token: token})
            .then(res => bartenderToken = res.token)
            .then(_ => app.services.setTemplate('home.html'))
            .then(_ => getOrders())
            .then(_ => getWS())
            .catch(_ => app.router.resolve('/login'))
        ;
        return true;
    };

    const getOrders = _ => {
        return app.services.http(true,true,bartenderToken).get('/orders')        
            .then(res => setOrders(res))
        ;
    };

    const getWS = _ => {
        return app.services.http(true,true,bartenderToken).get('/menu')
    ...
    const setWS = _ => {
        ...
        if (stopws) return;
        if (!etab_id) return;
        WS = new WebSocket('ws://ws.easy-as-pie.fr/orders?etabid=' + etab_id + '&authorization=' + bartenderToken);
        WS.onclose = _ => {
            WS = null;
    ...
    const updateOrder = (event, type) => {
        ...
        return app.services.http(true,true,bartenderToken).put('/update-order', data)
            .then(_ => getOrders());
    }
